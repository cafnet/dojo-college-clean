class UserMailer < ActionMailer::Base
  ...
  def invite_user(user, resource, role = nil)
    ...
    mail(to: @user.email, subject: @subject, from: "#{@resource.title} <info@dojo.college>")
  end
  ...
  def invite_member(user, resource, pitch)
    ...
    mail(to: @user.email, subject: @subject, from: "#{@pitch.program.title} <info@dojo.college>")
  end
  ...
  def message_all_pitch_members(user, feedback_reciever_id, resource, pitch,feedback, subject, feedback_id, presence)
    ...
    @presence = presence
    mail(to: @feedback_reciever.email, subject: @subject, from: "#{@program.title} <info@dojo.college>")
  end
  ...
  def welcome_user_message(user, role, welcome_message, program)
    ...
    @domain_host = DomainMapping.domain(@program)
    mail(to: @user.email, subject: @subject, from: "#{@program.title} <info@dojo.college>")
  end
  ...
  def invite_admin(user, resource, role = nil, invitation_message,subject, invitor_id)
    ...
    mail(to: @user.email, subject: @subject, from: "#{@resource.company_name} <info@dojo.college>")
  end
  ...
  def admin_announcement_mail(member, community_feed, user, program)
    ...
    mail(to: @member.email, subject: @subject, from: "#{@program.title} <info@dojo.college>")
  end
  ...
  def mentor_offer_mail(user,resource,pitch,member,message,subject)
    ...
    mail(to: @member, subject: @subject, from: "#{@pitch.program.title} <info@dojo.college>")
  end
  ...
  def submit_project_mail(user,resource,pitch,message,subject)
    ...
    mail(to: [emails], subject: @subject, from: "#{@pitch.program.title} <info@dojo.college>", reply_to: pitch.user.try(:email))
  end
  ...
  def activity_feeds_mail(program_id, user_id, mail_setting)
    ...
    if !@feeds.blank?
      mail(to: @member.email, subject: "Activity Feeds of #{@program.title}", from: "#{@program.title} <info@dojo.college>")
    end
  end
  ...
  def project_feeds_mail(pitch_id, user_id, mail_setting)
    ...
    @sub_domain = DomainMapping.sub_domain?(@domain_host)
    if !@feeds.blank?
      mail(to: @member.email, subject: "Project Feeds of #{@pitch.title}", from: "#{@pitch.title} <info@dojo.college>")
    end
  end
  ...
  def bug_reporting_message(email, subject, description, organisation)
    ...
    mail(to: "bugs@dojo.college", subject: subject, from: email, reply_to: email)
  end
  ...
  def message_to_admin(user_id, role, program_id)
    ...
    admin_emails.each do |admin|
      mail(to: admin, subject: @subject, from: "#{@program.title} <info@dojo.college>").deliver
    end
  end
  ...
  def user_reminder_mail(program_id, user_id, course_setting_id)
    ...
    mail(to: @member.email, subject: "Inactive from more than #{inactive_duration} for #{@program.title}", from: "#{@program.title} <info@dojo.college>")
  end
  ...
  def refer_pitch_to_person(program_id, pitch_id, mentor_name, mentor_email, invitation_msg, from)
    ...
    @domain_host = DomainMapping.domain(@program)
    @sub_domain = DomainMapping.sub_domain?(@domain_host)
    mail(to: mentor_email, subject: "#{@from.first_name} #{@from.last_name} is inviting you to join #{@program.title} as a #{Semantic.t(@program, 'role_type:mentor')} for #{@program.title}", from: "#{@program.title} <info@dojo.college>", reply_to: @from.try(:email)).deliver
  end
  ...
  def collaboration_request(user_id, pitch_id, member, invitation_msg)
    ...
    @sub_domain = DomainMapping.sub_domain?(@domain_host)
    @member = member
    mail(to: @member["email"], subject: "Collaboration request from #{@from.try(:first_name).try(:titleize)} #{@from.try(:last_name).try(:titleize)}", from: "#{@program.title} <info@dojo.college>", reply_to: @from.try(:email)).deliver
  end
  ...
  def accept_collaboration_request(pitch_id, accepted_by, user_id)
    ...
    @domain_host = DomainMapping.domain(@program)
    @sub_domain = DomainMapping.sub_domain?(@domain_host)
    mail(to: [@user.email, @from.try(:email)], subject: "Collaboration request accepted: #{@user.try(:first_name).try(:titleize)} meet #{@from.try(:first_name).try(:titleize)}", from: "#{@program.title} <info@dojo.college>").deliver
  end
  ...
  def accept_collaboration_request_cc(pitch, accepted_by, user, resource, message, subject)
    ...
    #all_team_members  << @pitch.user
    emails = all_team_members.flatten.map(&:email)
    mail(to: [@user.try(:email), @from.try(:email)], cc: emails, subject: @subject, from: "#{@program.title} <info@dojo.college>").deliver
  end
  ...
  def reject_collaboration_request(pitch_id, rejected_by, user_id)
    ...
    @user= User.where(id: user_id).first
    mail(to: @user.email, subject: "Collaboration request declined", from: "#{@program.title} <info@dojo.college>", reply_to: @from.try(:email)).deliver
  end
  ...
  def send_welcome_message_to_admin(user_id, org_id)
    ...
    mail(to: @user.try(:email), bcc: "getintouch@dojo.college", subject: "Thanks for choosing Apptual", from: "info@dojo.college")
  end

  def send_approval_message_to_admin(user_id, org_id)
    @user = User.where(id: user_id).first
    @organisation = Organisation.where(id: org_id).first
    mail(to: @user.try(:email), subject: "Welcome to Apptual", from: "info@dojo.college")
  end
  ...
  def send_invite_mentor_response(pitch_id, mentor_id, response, resource)
    ...
    mail(to: team_emails, subject: @subject, from: "#{@pitch.program.title} <info@dojo.college>").deliver
  end
  ...
  def send_mentor_offer_response(pitch_id, mentor_id, response, resource)
    ...
    @domain_host = DomainMapping.domain(@pitch.program)
    @sub_domain = DomainMapping.sub_domain?(@domain_host)
    mail(to: @mentor.email, subject: @subject, from: "#{@pitch.program.title} <info@dojo.college>").deliver
  end
  ...
  def voting_sheet_request(user_id, prog_id, folder_name)
    ...
    mail(to: @user.email, subject: @subject, from: "#{@program.title} <info@dojo.college>").deliver
  ...
  def send_participant_rejection_mail(event_record_id, subject, message)
    ...
    mail(to: @user.email, subject: subject, from: "#{@program.try(:title)} <info@dojo.college>")
  end
  ...
  def immediate_post_mail(member, community_feed, user, program, pitch)
    ...
    @subject = (@community_feed.subject.blank?)?(@default_subject):(@community_feed.subject)
    mail(to: @member.email, subject: @subject, from: "#{@pitch.blank? ? @program.try(:title) : @pitch.try(:title)} <info@dojo.college>" )
  end
  ...
  def send_report(user, report, prog_id)
    ...
    mail(to: @user.email, subject: @subject, from: "info@dojo.college")
  end
